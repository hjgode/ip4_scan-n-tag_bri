...
#region NewStuff
        ...
        public static string BinStr2HexStr(string BinStr)
        {
            ...
            while (sBin.Length > 0)
            {
                ...
                sHex2 = Convert.ToString(Convert.ToInt32(sBin8,2), 16); //0x53
        ...
        public static string HexStr2BinStr(string hex)
        {
            ...
            if (hex.Length == 24)
            {
                for (int i = 12; i > 0; i--)
                {
                    hex2 = hex.Substring((i - 1) * 2, 2);//start at right
                    BinStr = BinStr + Convert.ToString(Convert.ToInt32(hex2, 16), 2).PadLeft(8, '0');
                }
            }
            return BinStr;
        }
